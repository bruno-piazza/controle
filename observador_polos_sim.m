...
title("Velocidade angular: velocidade inicial não nula")
xlabel("Tempo [s]")
ylabel("Velocidade angular [rad/s]")
...
plot(t,Torq2)
plot(t,Torq3)
title(["Torque requerido para estabilização-","alocação de polos"])
xlabel("Tempo [s]")
ylabel('Torque [N.m]')
legend('T_{rr1}','T_{rr2}','T_{rr3}','Location','east')
hold off
grid on
baseFileName = sprintf('Image_%s.png', "aloc_pol_torque");
fullFileName = fullfile("Imagens\Controle Moderno", baseFileName);
saveas(6, fullFileName);